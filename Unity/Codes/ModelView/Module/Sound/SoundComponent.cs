    ...
    public class SoundComponent : Entity, IAwake
    {
        ...
        public void Awake()
        {
            ...
            if (root == null)
            {
                ...
                root.gameObject.AddComponent<AudioListener>();
        ...
        private async ETTask<SoundData> LoadSound(string soundName)
        {
            ...
            {
                //GameObject sound = (GameObject)resourcesComponent.GetAsset("sound.unity3d", "Sound");
                //GameObject prefab = sound.Get<GameObject>(soundName);
                //if (prefab == null)
                //{
                //    GameObject words = (GameObject)resourcesComponent.GetAsset("words.unity3d", "Words");
                //    prefab = words.Get<GameObject>(soundName);
                //}
                //if (prefab != null)
                //{
                //    GameObject sobj = GameObject.Instantiate((GameObject)prefab);
                //    SoundData sounddata = sobj.GetComponent<SoundData>();
                //    abSounds.Add(soundName, sounddata);
                //}
                //else
                //{
                //    Log.Error($"sound words 没有此音效 { soundName}");
                //}
            }
        ...
        {
            foreach (string name in _allClips[SoundType.Music].Keys)
            {
                this.Pause(name);
            }
        }
